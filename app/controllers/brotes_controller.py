...
from app.models.indicadores_model import IndicadoresModel
import logging
import zipfile
import io, os
...
@brotes_bp.route('/api/unidad_por_clues/<clues>', methods=['GET'])
@login_required
def obtener_unidad_por_clues(clues):
    try:
        unidad = BroteModel.obtener_unidad_por_clues(clues)
        if unidad:
            return jsonify({
                'success': True, 
                'data': {
                    ...
                    'institucion': unidad['nombre_institucion'],
                    'jurisdiccion': unidad['jurisdiccion']
                }
            })
        return jsonify({'success': False, 'message': 'CLUES no encontrado'})
    except Exception as e:
...
@login_required
@rol_requerido('super_administrador', 'jefe_departamento', 'coordinador_estatal', 'consultor_externo')
def lista_brotes():   
    ...
    anios = BroteModel.obtener_anios_disponibles() 
    ...
    if anio is None or str(anio).strip() == "":
        ...
        anio = anio_actual if anio_actual in anios else (anios[0] if anios else anio_actual)

    if fecha_inicio and fecha_fin:
        brotes = BroteModel.obtener_brotes_por_fecha_inicio(fecha_inicio, fecha_fin, anio)
    else:        
        brotes = BroteModel.obtener_brotes_por_anio(anio)        
...
def exportar_excel_lista():    
    ...
    if fecha_inicio_str and fecha_fin_str:
        try:
            ...
            logger.warning("Formato de fecha inválido en exportación")
            fecha_inicio = None
            fecha_fin = None
    ...
    try:        
        ...
        filename = f'brotes_listado_nominal_{anio}_{timestamp}.xlsx'
...
def exportar_excel_alta_pendientes():
    ...
    brotes = BroteModel.obtener_edo_actual_pendientes(anio)
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f'brotes_pendiente_alta_{anio}_{timestamp}.xlsx'
...
def exportar_excel_activos():
    ...
    brotes = BroteModel.obtener_edo_actual_activos(anio)
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f'brotes_activos_{anio}_{timestamp}.xlsx'
...
def dashboard():    
    try:
        ...
        instituciones = BroteModel.obtener_instituciones()
        datos = BroteModel.obtener_estadisticas(institucion, anio)
        resumen = BroteModel.obtener_resumen_instituciones(institucion, anio)   
        resumen_eventos = BroteModel.obtener_resumen_eventos(institucion, anio)
        resumen_diagnosticos = BroteModel.obtener_resumen_diagnosticos(institucion, anio)
        resumen_jurisdicciones = BroteModel.obtener_resumen_jurisdiccion(institucion, anio)
        diagnosticos = BroteModel.obtener_diagnosticos(institucion, anio)
        totales = BroteModel.obtener_totales_brotes(institucion, anio)
        
        tipos_eventos = sorted({
            key
            for fila in resumen_eventos
            for key in fila.keys()
            if key not in ["idinstitucion", "institucion", "total"]
        })
        ...
        return render_template('brotes/dashboard.html', 
                            ...
                            resumen_diagnosticos=resumen_diagnosticos,
                            resumen_jurisdicciones=resumen_jurisdicciones,
...
#Endpoint para actualizar dinamicamente los datos
@brotes_bp.route("/dashboard/data")
def dashboard_data():
    institucion = request.args.get("institucion", type=int)
    anio = request.args.get("anio", type=int)
...
#End point para datos de grafico
@brotes_bp.route("/dashboard/semanas")
@login_required
def dashboard_semanas():

    institucion = request.args.get("institucion", type=int)
    anio = request.args.get("anio", type=int)
    diagnostico = request.args.get("diagnostico", type=int)

    semanas = BroteModel.obtener_brotes_por_semana(
        institucion,
        anio,
        diagnostico
    )
...
def brotes_completos():
    ...
    return render_template(
        ...
        anios=anios, 
        anio_seleccionado=anio,              
        titulo=f"Listado Nominal de Brotes {anio}",
        encabezado=f"Listado Nominal de Brotes {anio}",
        export_url=url_for('brotes_bp.exportar_excel_lista', anio=anio)
    )
...
def brotes_pendientes():
    ...
    return render_template(
        ...
        anios=anios,
        anio_seleccionado=anio, 
        titulo=f"Brotes pendientes de alta {anio}",
        encabezado=f"Brotes pendientes de alta {anio}",
        export_url=url_for('brotes_bp.exportar_excel_alta_pendientes', anio=anio)
    )
...
def brotes_activos():
    ...
    return render_template(
        ...
        anios=anios, 
        anio_seleccionado=anio, 
        titulo=f"Brotes activos {anio}",
        encabezado=f"Brotes activos {anio}",
        export_url=url_for('brotes_bp.exportar_excel_activos', anio=anio)
    )



@brotes_bp.route('/exportar_documentos')
@login_required
def exportar_documentos():
    ...
    try:
        ...
        if not anio:
            return "Año requerido", 400

        if fecha_inicio_str and fecha_fin_str:
            fecha_inicio = datetime.strptime(fecha_inicio_str, "%Y-%m-%d").date()
            fecha_fin = datetime.strptime(fecha_fin_str, "%Y-%m-%d").date()

        # 🔍 Obtener brotes según filtro
        if fecha_inicio and fecha_fin:
            brotes = BroteModel.obtener_brotes_por_fecha_inicio(fecha_inicio, fecha_fin, anio)
        else:
            brotes = BroteModel.obtener_brotes_por_anio(anio)
        ...
        if not brotes:
            return "No hay documentos para exportar", 404
        ...
        with zipfile.ZipFile(memory_file, 'w', zipfile.ZIP_DEFLATED) as zf:
            ...
            for brote in brotes:
                ...
                folio = brote.get('folio_anual')
                carpeta_brote = f"brote_{folio}"

                ruta_brote = os.path.join(
                ...
                if not os.path.exists(ruta_brote):
                    continue

                for root, dirs, files in os.walk(ruta_brote):
                    for file in files:
        ...
        nombre_zip = f"documentos_brotes_{anio}.zip"
        ...
        return "Error interno al generar ZIP", 500



@brotes_bp.route('/indicador_oportunidad', methods=['GET'])
@login_required
def indicador_oportunidad():
    return render_template(
        'indicadores/oportunidad.html',
        titulo="Indicador de Notificación Oportuna",
        encabezado="Indicador de notificación oportuna por institución")


@brotes_bp.route('/api/indicador_oportunidad', methods=['GET'])
@login_required
def api_indicador_oportunidad():
    ...
    if fecha_inicio > fecha_fin:
        return jsonify({"error": "La fecha inicio no puede ser mayor a la fecha final"}), 400

    resultados = IndicadoresModel.obtener_oportunidad_triple(
        fecha_inicio,
        fecha_fin
    )
