...
def login():
    ...
    if request.method == 'POST':
        ...
        current_app.logger.info(f"Intento de inicio de sesión con el correo: {email}")
        ...
        try: 
            ...
            if user and check_password_hash(user.password_hash, password):
                ...
                current_app.logger.warning(f"Failed login attempt for email: {email} - Invalid credentials")
                flash('Correo o contraseña incorrectos.', 'danger')
                return redirect(url_for('auth_bp.login'))
        
        except Exception as e:
            # Log de errores inesperados
            current_app.logger.error(f"Error during login for email: {email} - {str(e)}", exc_info=True)
